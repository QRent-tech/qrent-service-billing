...
public class ContractUseCaseService implements ContractAddUseCase, ContractUpdateUseCase {
  ...
  public void update(final ContractUpdateRequest request) {
    ...
    final var today = qDateTime.getNow();
  ...
  private void stop(final Contract contractToStop, final LocalDateTime stopDate) {
    contractToStop.setDateEnd(stopDate.toLocalDate());
    updatePort.update(contractToStop);
  }

  private ContractAddRequest getContractAddRequest(
      final Contract contractToStop, final Long newQFirmId, final LocalDateTime startDate) {
