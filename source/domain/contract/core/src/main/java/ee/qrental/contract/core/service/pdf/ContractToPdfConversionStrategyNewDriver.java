...
public class ContractToPdfConversionStrategyNewDriver implements ContractToPdfConversionStrategy {
  ...
  public InputStream getPdfInputStream(final ContractPdfModel model) {
   ...
   pdfDocument.add(chapter1);
    ...
    chapter2.addCell(
            getSubChapterText("Rentnik kohustub oma eelmise perioodi võlakohustuste katteks tasuma kehtivale nädalaüüri hinnale täiendavalt 25%, kui sellised võlakohustused on olemas."));
    chapter2.addCell(getSubChapterNumber("2.6"));
    chapter2.addCell(
            getSubChapterText("Rendileandja võib nõuda rentniku renditasu või tema renditasu ettemaksu taksotellimusi või muid tellimusi vahendamisel osalevalt ettevõttelt (näiteks Bolt või Forus)."));
   ...
   chapter4.addCell(
    ...
    chapter13.addCell(
            getSubChapterText(
                    "Rentnikul on õigus leping igal ajal mistahes põhjusel lõpetada. "));
